...
## ADR-011: Phase 5 evaluation results (2026-04-30, revised 2026-05-03)
**Decision**: Random Forest is the lead model based on combined IC,
ICIR, regime stability, and held-out evaluation. Linear baseline shows
surprising portfolio Sharpe leadership but regime instability;
...
**Walk-forward IC by year**:
  ...
  All four models lose skill in 2023. The cause is more nuanced than
  "the AI rally": 2023 is the visible point at which a four-year
  structural narrowing of the S&P 100 cross-section caught up with
  factor-style models. The chain runs:
 
  - **2020 COVID shock**: pandemic accelerated digital adoption
    asymmetrically toward mega-caps. Cloud, e-commerce, streaming,
    remote-work tooling all permanently re-rated. AAPL/MSFT/GOOGL/
    META/AMZN/NVDA/TSLA captured most of the structural shift because
    they own the rails. Cross-section starts concentrating.
  - **2020-2021 zero-rate amplifier**: long-duration assets revalue
    upward. COVID winners get a second tailwind. Cross-section
    concentration deepens.
  - **2022 rate-cycle filter**: rapid hikes punish growth, but the
    COVID-winners that became cash-flow machines survive better than
    smaller growth peers. Within tech itself, winners pull away.
  - **2023 AI narrative**: ChatGPT (Nov 2022) and the generative AI
    hype layer onto an *already-concentrated* cross-section. NVDA
    chips, MSFT/GOOGL platforms, META compute spend dominate index
    return.
  Factor models work best when the cross-section has healthy
  dispersion. They struggle when 7 names drive most of the index.
  The 2023 IC break is the *culmination* of this 4-year process,
  not its cause.
...
unbalanced — bull/low_vol has 355 days, bear/high_vol 194,
bull/high_vol 73, bear/low_vol just 8 days. The bear/low_vol cell
is unreliable (SE of mean Spearman ≈ 0.30 at n=8); we exclude it
from interpretation. The 73-day bull/high_vol cell warrants caveats
but is interpretable. The two well-populated cells (bull/low_vol and
bear/high_vol) carry the substantive findings.
 
This imbalance is itself a finding: the test period (Feb 2021 - Nov
2023) was dominated by bull/low_vol regime (355 days, 56% of total),
with bear/high_vol the natural "second mode" (194 days, 31%). The
other two cells are transition periods, rare by construction.
 
**SHAP stability** (data/processed/shap_ranks_rf.csv):
...
**Decile long-short portfolio (32 non-overlapping 20-day periods)**:
  ...
  | Model  | Sharpe (0bps) | Sharpe (5bps) | Sharpe (10bps) |
  |--------|---------------|---------------|----------------|
  ...
  IC and Sharpe rankings disagree: Linear has the best tails (largest
  top-decile minus bottom-decile spread) despite weakest IC. This
  reflects different aspects of model behavior — IC measures
...
**Hold-out evaluation (RF trained on 2018-2023, evaluated 2024-2026)**:
  - 2024 mean IC: -0.021 (post-COVID concentration unwinds slowly;
    mega-cap dominance still suppresses cross-sectional dispersion)
  - 2025 mean IC: +0.017 (cross-section re-broadens; factor signals
    partially recover)
  ...
  effects (Fold 1 hyperparameter tuning; folds 1-5 within a single
  macro regime). The 2024 negative IC is the project's most
  important caveat — and is consistent with the structural-narrowing
  story documented above. The 2025 partial recovery aligns with
  cross-section broadening as concentration unwinds.
 
**Reasoning for RF lead despite Linear's higher Sharpe**:
  - RF has positive IC in all 4 regime cells; Linear has 2 negative
    cells.
  ...
  - Sharpe estimate on 32 periods has SE ~0.3; the 0.40 spread between
    Linear and RF Sharpe is barely outside one SE.
  - Regime robustness matters more for production deployment than a
    one-off Sharpe number.
**What we'd do differently in v2**:
  - Tighter regime coverage in training (2008-2015 history with
    point-in-time S&P 100 membership) — would give the model more
    examples of *narrowing* and *broadening* cycles.
  - Explicit regime-conditional models (separate model per
    trend×vol cell, switched at predict time).
  - More frequent retraining cadence than quarterly to keep up with
    the apparent factor decay shown in 2024 hold-out.
  - Wider universe (S&P 500) for tighter tail estimates.
**Revision history**:
  - 2026-04-30: original ADR. Attributed 2023 break primarily to
    "the AI rally."
  - 2026-05-03: revised causal framing. The 2023 break is the
    visible point of a four-year structural narrowing of the
    cross-section that began with the 2020 COVID shock. The AI
    narrative is the catalyst, not the cause.
